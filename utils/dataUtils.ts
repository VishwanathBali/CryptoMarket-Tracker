...
export const fetchAndStoreData = async () => {
  try {
    ...
    for (const stock of data) {
      ...
      const existingPrices = JSON.parse(localStorage.getItem(name) || "[]");
      ...
      existingPrices.push(newPriceEntry);

      if (existingPrices.length > 20) {
        existingPrices.shift();
      }

      localStorage.setItem(name, JSON.stringify(existingPrices));
...
export const fetchStockData = async (crypto: string) => {
  try {
    ...
    const data = await response.json();
    return { name: crypto, prices: data.prices };
  } catch (error) {
    console.error("Error fetching data:", error);
    return { name: crypto, prices: [] };
  }
};
